    ...
    return (
      ...
      <Container>
        ...
        <ComponentsContainer>
          ...
          <StudyComponent
            title={components[this.state.index].title}
            image={components[this.state.index].image}
            logo={components[this.state.index].logo}
            onPress={() => {
              this.props.navigation.push(selected.screenStudy, {
                selectedcomponent: component
              });
            }}
          />
          <StudyComponent
            title={components[getNextIndex(this.state.index)].title}
            image={components[getNextIndex(this.state.index)].image}
            logo={components[getNextIndex(this.state.index)].logo}
            onPress={() => {
              this.props.navigation.push(selected.screenGroupChat, {
                selectedcomponent: component
              });
            }}
          />
          <StudyComponent
            title={components[getNextIndex(this.state.index + 1)].title}
            image={components[getNextIndex(this.state.index + 1)].image}
            logo={components[getNextIndex(this.state.index + 1)].logo}
            onPress={() => {
              this.props.navigation.push(selected.screenGroupChat, {
                selectedcomponent: component
              });
