...
class AccountScreen extends React.Component {
  ...
  opencloseMenu = () => {
    if (this.props.action == "openMenu") {
      ...
      Animmted.spring(this.state.opacity, { toValue: 0.5 }).start();
    }

    if (this.props.action == "closeMenu") {
      Animated.spring(this.state.scale, { toValue: 1 }).start();
      Animmted.spring(this.state.opacity, { toValue: 1 }).start();
    }
  };
    ...
    return (
      <RootView>
        ...
        <AnimatedContainer
          style={{
            transform: [
              { scale: this.state.scale },opacity: this.state.opacity
            ]
          }}
        >
          <SafeAreaView>
